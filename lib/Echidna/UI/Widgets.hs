...
campaignStatus :: MonadReader UIPrinterInfo m => UIState -> m (Widget Name)
campaignStatus uiState = do
  done <- runReader (isDone uiState.campaign) <$> asks (._cfg)
...
testsWidget :: MonadReader UIPrinterInfo m => [EchidnaTest] -> m (Widget Name)
testsWidget tests' =
...
testWidget :: MonadReader UIPrinterInfo m => EchidnaTest -> m (Widget Name)
testWidget test =
...
tsWidget :: MonadReader UIPrinterInfo m
...
tsWidget (Open i)   t = do
  n <- asks (._cfg.campaignConf.testLimit)
  if i >= n then
    tsWidget Passed t
  else
...
failWidget :: MonadReader UIPrinterInfo m
...
optWidget :: MonadReader UIPrinterInfo m
          => TestState -> EchidnaTest -> m (Widget Name, Widget Name)
...
maxWidget :: MonadReader UIPrinterInfo m
...
seqWidget :: MonadReader UIPrinterInfo m => [Tx] -> m (Widget Name)
seqWidget xs = do
